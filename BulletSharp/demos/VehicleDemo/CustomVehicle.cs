    ...
    public sealed class CustomVehicle : IAction, IDisposable
    {
        ...
        public void DebugDraw(DebugDraw debugDrawer)
        {
            for (int v = 0; v < NumWheels; v++)
            {
                ...
                Vector3 axle = transform.GetColumn(RightAxis);
        ...
        private void ResolveSingleBilateral(RigidBody body1, Vector3 pos1, RigidBody body2, Vector3 pos2, float distance, Vector3 normal, ref float impulse, float timeStep)
        {
            ...
            Matrix4x4 world2A = Matrix4x4.Transpose(body1.CenterOfMassTransform.GetBasis());
            Matrix4x4 world2B = Matrix4x4.Transpose(body2.CenterOfMassTransform.GetBasis());
            ...
            float dot0 = Vector3.Dot(m_0MinvJt, m_aJ);
            float dot1 = Vector3.Dot(m_1MinvJt, m_bJ);
        ...
        public void UpdateFriction(float timeStep)
        {
            ...
            for (int i = 0; i < NumWheels; i++)
            {
                ...
                if (groundObject != null)
                {
                    ...
                    _axle[i] = wheelTrans.GetColumn(indexRightAxis);

                    Vector3 surfNormalWS = wheel.RaycastInfo.ContactNormalWS;
                    float proj = Vector3.Dot(_axle[i], surfNormalWS);
                    _axle[i] = Vector3.Normalize(_axle[i] - surfNormalWS * proj);

                    _forwardWS[i] = Vector3.Normalize(Vector3.Cross(surfNormalWS, _axle[i]));
            ...
            for (int i = 0; i < NumWheels; i++)
            {
                ...
                if (_sideImpulse[i] != 0)
                {
                    ...
                    Vector3 vChassisWorldUp = new Vector3(
                        RigidBody.CenterOfMassTransform.GetComponent(0, indexUpAxis),
                        RigidBody.CenterOfMassTransform.GetComponent(1, indexUpAxis),
                        RigidBody.CenterOfMassTransform.GetComponent(2, indexUpAxis));
        ...
        public void UpdateVehicle(float step)
        {
            ...
            Vector3 forwardW = chassisTrans.GetColumn(indexForwardAxis);
